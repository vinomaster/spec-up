#  Contributor's Guide
This document provides instructions for developing specifications using the **ToIP Spec-Up Theme**.
...
1.  Launch the development environment. Iteratively develop documentation content by editing the markdown files. Each time you save a file, the test server will automatically refresh content for your review using your web browser pointed at: [localhost](localhoat:8800). See your `Makefile` for actual PORT number.
...
3. Exit the Develop and Test environment (container) when you are finished testing.
...
#### Generate Markdown and PDF Docs
This repo generates a single HTML file from onr oer more Markdown files. The following publication process will use the HTML file to produce a single Markdown file as well as a PDF file.
...
2. Run the publishing process which will generate various versions of the content in the `./publish` folder.
...
#### Merge with Template
If the original `spec-up` template repo has been changed, you can use the following command to update this repo. This will ensure that your ```fork``` and ```local-machine``` are in sync with changes made to the ```template``` repo.
